...
int main(int argc, char* argv[])
{
  ...
  {
	/* create threads to execute accelerate_flow and propagate*/
	#pragma omp parallel 
	{		
    accelerate_flow(params,cells,obstacles);
    propagate(params,cells,tmp_cells);
	}
    /* sychronize threads here */
...
int propagate(const t_param params, t_speed* cells, t_speed* tmp_cells)
{
  ...
  {
	/* determine indices of y-axis-direction neighbours
	...
	for (jj = 0; jj < params.nx; jj++)
    {
      ...
      /* determine indices of x-axis-direction neighbours
      ** respecting periodic boundary conditions (wrap around) */
...
double collision(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles)
{
  ...
  /* compute all the intensive double precisions and store them in const variables*/
  const double const1 = 1 / c_sq , const2 = 1 / (2.0 * c_sq) , const3 = 1 / (2.0 * c_sq * c_sq) ;
  ...
  ** are in the scratch-space grid */
  ...
  /* create threads to execute the for loop in parallel*/
  ...
  { 
	...
	/* don't consider occupied cells (if the cell does not contain an obstacle then collide */
      else
      {
        ...
        /* velocity squared */
        double u_sq = pow(u_x,2) + pow(u_y,2);
        ...
        double d_equ[NSPEEDS], w11=w1 * local_density, w22= w2 * local_density, u_sq1= 1.0 - u_sq*const2;
        ...
        for (int kk = 0; kk < NSPEEDS; kk++)
        {		  	
          cells[ii].speeds[kk] = tmp_cells[ii].speeds[kk] + params.omega * (d_equ[kk] - tmp_cells[ii].speeds[kk]);
        }
