...
#include <stdlib.h>
 ...
 void save_table(int *X, int WX, int WY){
  FILE *fp;
  char filename[20];
  sprintf(filename, "cuda_gmc_table%dx%d.bin", WX, WY);
 ...
 // Iterate data chunk of this thread
    for ( int id_x =x; id_x<WX ; id_x += blockDim.x * gridDim.x)
    { for ( int id_y = y; id_y<WY ; id_y += blockDim.y * gridDim.y){
	
	int id=getId(id_x, id_y, WX, WY);
 ...
 for(int x_offset=-1; x_offset<=1; x_offset++){
	for(int y_offset=-1; y_offset<=1; y_offset++){
		if(x_offset != 0 || y_offset != 0){ // don’t count itself
			int neighborId = getId(id_x + x_offset, id_y + y_offset, WX, WY);
 ...
 dim3 gridSize(BLOCK_SIZE,BLOCK_SIZE, 1); 
 ...
 printf("Elapsed time (Global Memory data per thread) : %f ms\n" ,elapsedTime);
