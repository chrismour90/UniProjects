...
for i=2:size(R,1)-1
    for j=2:size(R,2)-1
       if (R(i,j)==0)
        if (mod(i,2)==0)
        R(i,j)=R(i,j-1)/2+R(i,j+1)/2;   
        end
        if (mod(i,2)~=0 && mod(j,2)~=0)
        R(i,j)=R(i-1,j)/2+R(i+1,j)/2;   
        end
        if (mod(i,2)~=0 && mod(j,2)==0)
        R(i,j)= R(i-1,j-1)/4 + R(i+1,j+1)/4 + R(i-1,j+1)/4 + R(i+1,j-1)/4;
        end
       end 
    end
end    
...
for i=2:size(B,1)-1
    for j=2:size(B,2)-1
        if (B(i,j)==0)
        if (mod(i,2)~=0)
        B(i,j)=B(i,j-1)/2+B(i,j+1)/2;   
        end
        if (mod(i,2)==0 && mod(j,2)==0)
        B(i,j)=B(i-1,j)/2+B(i+1,j)/2;   
        end
        if (mod(i,2)==0 && mod(j,2)~=0)
        B(i,j)= B(i-1,j-1)/4 + B(i+1,j+1)/4 + B(i-1,j+1)/4 + B(i+1,j-1)/4;
        end
       end 
    end
end    
...
for i=2:size(G,1)-1
    for j=2:size(G,2)-1
        if (G(i,j)==0)
        G(i,j)= G(i+1,j)/4 + G(i-1,j)/4 + G(i,j-1)/4 + G(i,j+1)/4;
        end
   end
end    
